...
Layer *make_convolutional_layer_gpu(int filters, int ksize, int stride, int pad, int bias, int normalization, char *active)
{
    ...
    l->init = init_convolutional_layer_gpu;
    l->release = release_convolutional_layer_gpu;
    ...
    fprintf(stderr, "Convolutional   Layer    :    [filters=%2d, ksize=%2d, stride=%2d, pad=%2d, bias=%d, normalization=%d, active=%s]\n",
            l->filters, l->ksize, l->stride, l->pad, l->bias, l->batchnorm, active);
    return l;
}
...
void weightinit_convolutional_layer_gpu(Layer *l, WeightInitType type)
{
    ...
    free(kernel_weights);
    if (l->bias){
        fill_gpu(l->bias_weights, l->filters, 0.001, 1);
