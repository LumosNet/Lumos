...
void xor () {
    ...
    create_train_scene(sess, 28, 28, 1, 1, 10, xor_label2truth, "/usr/local/lumos/data/mnist/train.txt", "/usr/local/lumos/data/mnist/train_label.txt");
    init_train_scene(sess, 500, 20, 4, NULL);
    session_train(sess, 0.1, "./lumos.w");

    Session *t_sess = create_session();
    bind_graph(t_sess, graph);
    create_test_scene(t_sess, 28, 28, 1, 1, 10, xor_label2truth, "/usr/local/lumos/data/mnist/train.txt", "/usr/local/lumos/data/mnist/train_label.txt");
