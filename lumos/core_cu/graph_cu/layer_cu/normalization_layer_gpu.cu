...
void backward_normalization_layer_gpu(Layer l, float rate, int num, float *n_delta)
{
    for (int i = 0; i < num; ++i){
        ...
        float *mean = l.mean + i*l.output_c;
        float *variance = l.variance + i*l.output_c;
        gradient_normalize_mean_gpu(delta_n, variance, l.output_h, l.output_w, l.output_c, l.workspace);
        gradient_normalize_variance_gpu(input, delta_n, mean, variance, l.output_h, l.output_w, l.output_c, l.workspace+l.output_c);
        gradient_normalize_gpu(input, mean, variance, l.workspace, l.workspace+l.output_c, l.output_h, l.output_w, l.output_c, delta_n, delta_n);
    }
