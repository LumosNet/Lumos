...
void backward_normalization_layer_gpu(Layer l, float rate, int num, float *n_delta)
{
    ...
    for (int i = 0; i < num; ++i){
        ...
        gradient_normalize_mean_gpu(delta_n, variance, l.output_h, l.output_w, l.output_c, mean_delta);
        gradient_normalize_variance_gpu(delta_n, input, mean, variance, l.output_h, l.output_w, l.output_c, variance_delta);
        gradient_normalize_gpu(input, mean, variance, mean_delta, variance_delta, l.output_h, l.output_w, l.output_c, delta_n, delta_l);
    }
}
