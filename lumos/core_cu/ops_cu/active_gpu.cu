...
__global__ void activate_list_kernel(float *origin, int num, ActivateGpu activate)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if (index >= num) return;
    origin[index] = activate(origin[index]);
}

void activate_list_gpu(float *origin, int num, ActivateGpu activate)
{
    activate_list_kernel<<<(num+BLOCK-1)/BLOCK, BLOCK>>>(origin, num, activate);
}

__global__ void gradient_list_kernel(float *origin, int num, GradientGpu gradient)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if (index >= num) return;
    origin[index] = gradient(origin[index]);
}

void gradient_list_gpu(float *origin, int num, GradientGpu gradient)
{
    gradient_list_kernel<<<(num+BLOCK-1)/BLOCK, BLOCK>>>(origin, num, gradient);
}
