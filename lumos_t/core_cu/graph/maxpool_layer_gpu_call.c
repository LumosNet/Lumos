#include "maxpool_layer_gpu_call.h"

void call_forward_maxpool_layer_gpu(void **params, void **ret)
{
    int *ksize = (int*)params[0];
    int *h = (int*)params[1];
    int *w = (int*)params[2];
    int *c = (int*)params[3];
    int *num = (int*)params[4];
    float *input = (float*)params[5];
    float *output = (float*)params[6];
    int *index = (int*)params[7];
    float *input_g = NULL;
    float *output_g = NULL;
    int *index_g = NULL;
    Layer *l = make_maxpool_layer(ksize[0], ksize[0], 0);
    init_maxpool_layer(l, w[0], h[0], c[0]);
    cudaMalloc((void**)&input_g, l->inputs*sizeof(float));
    cudaMalloc((void**)&output_g, l->outputs*sizeof(float));
    cudaMalloc((void**)&index_g, l->outputs*sizeof(int));
    cudaMemcpy(input_g, input, l->inputs*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(output_g, output, l->outputs*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(index_g, index, l->outputs*sizeof(int), cudaMemcpyHostToDevice);
    l->input = input_g;
    l->output = output_g;
    l->maxpool_index = index_g;
    l->forward(*l, num[0]);
    cudaMemcpy(output, output_g, l->outputs*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(index, index_g, l->outputs*sizeof(float), cudaMemcpyDeviceToHost);
    cudaFree(input_g);
    cudaFree(output_g);
    cudaFree(index_g);
    ret[0] = output;
    ret[1] = index;
}

void call_backward_maxpool_layer_gpu(void **params, void **ret)
{
    int *ksize = (int*)params[0];
    int *h = (int*)params[1];
    int *w = (int*)params[2];
    int *c = (int*)params[3];
    float *rate = (float*)params[4];
    int *num = (int*)params[5];
    float *l_delta = (float*)params[6];
    float *n_delta = (float*)params[7];
    int *index = (int*)params[8];
    float *l_delta_g = NULL;
    float *n_delta_g = NULL;
    int *index_g = NULL;
    Layer *l = make_maxpool_layer(ksize[0], ksize[0], 0);
    init_maxpool_layer(l, w[0], h[0], c[0]);
    cudaMalloc((void**)&l_delta_g, l->inputs*sizeof(float));
    cudaMalloc((void**)&n_delta_g, l->outputs*sizeof(float));
    cudaMalloc((void**)&index_g, l->outputs*sizeof(int));
    cudaMemcpy(l_delta_g, l_delta, l->inputs*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(n_delta_g, n_delta, l->outputs*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(index_g, index, l->outputs*sizeof(float), cudaMemcpyHostToDevice);
    l->delta = l_delta_g;
    l->maxpool_index = index_g;
    l->backward(*l, rate[0], num[0], n_delta_g);
    cudaMemcpy(l_delta, l_delta_g, l->inputs*sizeof(float), cudaMemcpyDeviceToDevice);
    cudaFree(l_delta_g);
    cudaFree(n_delta_g);
    cudaFree(index_g);
    ret[0] = l_delta;
}
