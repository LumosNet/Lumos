...
void weightinit_convolutional_layer_gpu(Layer l, FILE *fp)
{
    ...
    else if (initcpt.initype == UNIFORM_I) convolutional_uniform_init_gpu(l, initcpt.min, initcpt.max);
    ...
    else if (initcpt.initype == KAIMING_UNIFORM_I) convolutional_kaiming_uniform_init_gpu(l, initcpt.a, initcpt.mode, initcpt.nonlinearity);
